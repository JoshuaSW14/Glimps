...
export class ResurfacingService {
  /**
   * Select a meaningful event to resurface
   * Phase 6: Event-level resurfacing
   * 
   * Algorithm prioritizes:
   * 1. Temporal significance (anniversaries, time-based patterns)
   * 2. Emotional significance (confidence score, title/summary keywords)
   * 3. Diversity (location changes, different types of experiences)
   * 4. Age (not too recent, not too old)
   */
  async selectEventToResurf(): Promise<ResurfacingResult | null> {
    logger.info('Starting event resurfacing selection');
    
    try {
      // Get all events older than 7 days
      const sevenDaysAgo = new Date();
      sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
      
      const allEvents = await eventRepository.listRecent(500);
      ...
      if (eligibleEvents.length === 0) {
        logger.info('No eligible events for resurfacing');
        return null;
      }
   ...
   * Phase 6: Notification from event summary
   */
  ...
  /**
   * Get daily resurfaced event (should be called once per day)
   * Phase 6: Event-based resurfacing
   * In production, this would be called by a cron job
   */
  async getDailyMemory(): Promise<ResurfacingResult | null> {
    const result = await this.selectEventToResurf();
    
    if (result) {
      // Log the resurfacing event
      await retrievalLogRepository.create({
        userQuery: 'DAILY_RESURFACING',
        memoryIds: [], // Phase 6: Events instead of memories
        searchMetadata: {
